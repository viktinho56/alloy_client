...
const FoodFeature = () => {
    let userProperties = ["S/N", "Food Name", "Food Description", "Availability", "Actions"];
    ...
    let { data: AllUsers } = useProducts();

    let { data, error, isLoading } = usePaginatedProducts(userQuery);
    console.log(data);
        ...
        if (searchText.length > 2) {
            ...
            setUsers(() =>
                AllUsers.filter((user: any) =>
                    user.productName.toLocaleLowerCase().startsWith(lowerCaseSearchText)
                )
            );
                ...
                <Box my={2}>
                    ...
                    >
                        {users && users.length > 0 && !isLoading && (
                            <FoodTable data={users} properties={userProperties} />
                        )}
