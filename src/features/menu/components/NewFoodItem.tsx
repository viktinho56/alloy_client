...
import { CreateProductApi } from "../services/userServices";
...
export default function NewFoodItem({ close }: any) {
    ...
    const onSubmit = async (data: any) => {
        ...
        if (avatar && avatar.name.length > 0) {
            ...
            data.avatarUrl = path;
            if (!uploadError) {
                let { response, error, isLoading } = await CreateProductApi(data);
