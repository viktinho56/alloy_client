...
const CategoryFeature = () => {
    let userProperties = ["S/N", "Name", "Description", "status", "Actions"];
    ...
    let { data: AllUsers } = useCategories();
        ...
        if (searchText.length > 2) {
            ...
            setUsers(() =>
                AllUsers.filter((user: any) =>
                    user.categoryName.toLocaleLowerCase().startsWith(lowerCaseSearchText)
                )
            );
                ...
                <Box my={2}>
                    ...
                    >
                        {users && users.length > 0 && !isLoading && (
                            <CategoriesTable data={users} properties={userProperties} />
                        )}
